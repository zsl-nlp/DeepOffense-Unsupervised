...
#阿拉伯
dfs = []
file_1 ='../code/examples/arabic/data/offenseval-ar-training-v1.tsv'
df1 = pd.read_csv(file_1,sep='\t')
data = df1.rename(columns={'tweet': 'text', 'subtask_a': 'labels'})[['text', 'labels']]
texts = data['text'].tolist()
labels_ = data['labels'].tolist()
len_num = data.shape[0]
train_data = []
...
file_2 = '../code/examples/arabic/data/offenseval-ar-dev-v1.tsv'
df2 = pd.read_csv(file_2,sep='\t')
data = df2.rename(columns={'tweet': 'text', 'subtask_a': 'labels'})[['text', 'labels']]
texts = data['text'].tolist()
labels_ = data['labels'].tolist()
test_data = []
for i in range(0,len_num):
...
# test_data = all_data[int(len_num*0.9):] 
...
def eval_epoch1():
    ...
    print([sample,args.load_model,en_,nums_,a,p,r,f])
